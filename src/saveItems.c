...
uintmax_t forEachSaveItemWithID(context_t *context, forEachSaveItemFunction f, uintmax_t userData, uintmax_t saveItemID)
{
	...
	for (intmax_t i = 0; i < context->saveItemCount; i++)
		...
		if (context->saveItemDataSize > 0xfffffffc)
			...
			{
// strange: CustomiserSave size must be floored to 8000 multiple as it is read in 8000 bytes blocks
// checked in LMSH2: size is hard coded as 80320 but true size is 80000, when reading, size is completely ignored, 10 * 8000 bytes are read
// games that have this length mismatch issue are:
// superheroes2 (80320 vs 80000)
// incredibles (80640 vs 80000)
// villains (96768 vs 96000)
			if (context->saveItemID1 == saveItemCustomiserSave)
				context->saveItemDataSize = context->saveItemDataSize / 8000 * 8000;

			context->saveItemDataOffset = context->saveItemOffset + 0x08;
